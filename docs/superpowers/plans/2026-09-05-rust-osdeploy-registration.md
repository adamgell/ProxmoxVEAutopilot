...
Composite(run_id,predecessor_id) FK(run_id,operation_id), no selfpredecessor.
...
- [ ] Pair migration entry with generic isolation in this same task before any public registration exposure. Preserve the historical NativePveVmBoot early return before cap validation; then reject cap zero, then return None for OsDeploy before database access for both claim APIs. Existing bound claim has no separate upfront fingerprint/version validation to preserve or invent. Generic start/cancel/finalize reject both typed families; continuation/heartbeat reject OsDeploy only, preserving native behavior, after authority locking and before state/lease writes. Generic reaper excludes OsDeploy in discovery and rechecks family under locks. Public append_command rejects raw OsDeploy after existing digest/version checks and before database writes; internal append_command_tx remains private for atomic registration. No public bypass boolean. Generic append_event remains EvidenceRecorded-only and cannot cause execution.
- [ ] Registration validates/serializes/restores fullplan and downwardconversion before any DB transaction writes, including desired diskserial<=20. Begintransaction, acquire existingnative:run lock, then existing sorted VMID/UUID/MACidentitylocks, then the global osdeploy:agent lock. Never introduce inverse lock order with oldnative. Use canonical agentlabel and matching existing VMreservationNativeVmPlandigest.
...
- [ ] Serialize public generic intake on the same native:run lock before its command/operation locks. Reject a run already containing OsDeploy operations with TypedWorkflowRequired using the foundation operations table, not a query requiring migration0004. This prevents same-run cross-family races with typedregistration while preserving foundation-onlysetup. Test both deterministic lock orders with separate pools: genericwins→typedConflict and no typedrows; typedwins→genericTypedWorkflowRequired and no additionalgenericrow. ExistingnonOSDeployfamilyaliases remain supported.
