...
For evaluator Waiting, use a nonterminal branch in this same method: append the typed waiting journal outcome, transition Running to Waiting, update the attempt to Waiting, retain its lease and return ExecutionState::Waiting. Do not insert a terminal native_decisions row or set completed_at. Repeated Waiting may append bound evidence/outcome but does not create a Waiting-to-Waiting state event. Operation and attempt states must agree so existing heartbeat/continuation checks remain valid. Cancellation wins over Waiting and settles stopped execution as Unknown through the native fenced path.
...
Expose `NativeController::new(store: PgStore, scheduler: Scheduler, fake: Arc<NativeFakePve>) -> Self`, `run_once(&mut self, operation_id: OperationId) -> Result<NativeProgress, NativeControllerError>`, and `reconcile_once(&mut self, operation_id: OperationId) -> Result<NativeProgress, NativeControllerError>`. The constructor deliberately accepts the concrete fake, not arbitrary `PveMutationPort` or a URL. `NativeProgress` is `Idle`, `Waiting`, or `Decided(ExecutionState)`; errors are fixed reason labels. Retain active grants in a private per-operation map; serialize calls per controller instance and never expose tokens in Debug/output.

Begin each bounded run_once call with current-authority reaping of expired leases, then reload the native operation. Only Pending enters the new-dispatch path below. For Running/Waiting owned by a retained grant, heartbeat and replace that grant, collect outcome evidence and call decide_native; never enter dispatch again. Without a retained grant (including process restart), do not adopt/reclaim live work: return Waiting until expiry/reaping makes it Unknown. Route Unknown only to reconcile_once. Remove retained grants on terminal decisions or fence loss. Test delayed completion across at least two calls, unchanged attempt/one dispatch, restart before expiry, and deadline expiry. Existing reaper use is confined to the owned local store; it is not authorization for any production sweep.

Implement this order for a new Pending operation in `run_once`:
...
`reconcile_once` only loads the original plan/attempt/dispatch/receipt, collects authoritative observations and invokes `reconcile_native_unknown`. It never calls a mutation method and has no live lease: do not heartbeat, claim or dispatch on this path. During owned Running/Waiting execution, heartbeat between short reads; stop issuing further execution work if heartbeat/continuation rejects authority or cancellation. Use a 2-second per-read timeout, no more than three read attempts with 100ms then 250ms delays, and the existing database execution deadline as the hard bound for active work; reconciliation has its own bounded observation call and may prove an outcome after that original deadline. The executable proof loops `run_once`/`reconcile_once` at 100ms up to 30 seconds, then exits nonzero with a fixed failure label if recovery remains pending; it never waits indefinitely or reports unfinished recovery as success.
