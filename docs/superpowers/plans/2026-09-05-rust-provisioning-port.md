...
- This is an explicit synthetic provisioning-v1 PVE contract. Integral binary K/M/G/T parsing and task conventions are fake fixtures, not claims about installed PVE semantics. New resize fixtures use the pinned upstream `resize` worker, not `qmresize`; the main wire checkpoint records the source and installed-version limitation. Real-wire support requires separately verified contracts and authorization.
...
- [ ] Receipt worker conventions: clone qmclone/sourceVMID; resize resize/targetVMID; bothstarts qmstart/targetVMID; stop qmstop/targetVMID; configurations synchronousaccepted only. Resize forms additionally carry the exact before-config digest. Central matcher used by evidence and later fake. A task success without exact postconditions is insufficient; another start's task cannot satisfy this start. Check wrongnode/worker/VMID/operation/attempt/hash, taskrunning/failure, receipt loss/substitution and stale/future clocks.
...
**Interfaces:** A sealed `ProvisioningFakePort: PvePreflightReadPort + native::sealed::FakeMutationCapability` provides async provisioning_vm_config(node,vmid), provisioning_identity(node,vmid), provisioning_media(node,storage), and submit_provisioning(&ProvisioningMutationRequestV1) -> Result<MutationReceipt,PveWriteError>. Identity returns Task2's neutral ProvisioningIdentitySnapshotV1, not an old or lossy VM config. Existing task/power/node/inventory/QGA interfaces remain. `osdeploy_adapter::pve_expectations(&OsDeployPlanV1) -> Result<ProvisioningExpectationsV1,ContractError>` derives, never independently overrides, every PVE subset input.
...
### Upstream compatibility amendment before Task 2

Main read the completed Task 2 interface proposal and pinned upstream source; see `.superpowers/sdd/2026-09-05-rust-provisioning-port/upstream-wire-checkpoint.md`. These amendments do not change Task 1's current fact/declaration scope or authorize Task 2 before its full API checkpoint.

- Ruling: retain the accepted broad declaration/fact serial representation, but require desired disk serial length at most 20 ASCII bytes for every new provisioning dispatch request, not just ConfigurePe. Task 3's downward `pve_expectations` conversion must reject longer desired disk serials before any PVE operation, with exact 20/21-byte boundary tests; future service intake must perform that conversion before registration/dispatch. Never truncate identity. This keeps metadata distinct from executable compatibility while rejecting an unusable workflow before cloning. Cost if wrong: a later explicit versioned compatibility change may admit additional valid serials; old declarations and old native contracts remain unchanged.
- Ruling: use the `resize` worker and captured digest for new resize requests. Configured disk size remains an observed invariant, never an effect of configuration. The shared fake must not grow a volume when configuring its serial. Cost if wrong for an installed PVE version: revise its versioned real transport contract after evidence, not task attribution globally.
- Ruling: accept the proposal's rich clone wrapper, neutral cross-family identity projection, and non-deserializable verified physical-stage baseline. Preserve strict deadline expiry, including reconciliation, in this phase. These values provide physical evidence only; the durable store remains the authority for predecessors and dispatch. Exact constructor fields and validation matrix still await the full main-authored Task 2 brief after Task 1 completion.

### Main Task 3 interface decisions

Main read the complete bounded task-3-interface-proposal.md after reviewing the accepted Task2 contract and inspected existing insert/replace, completion and QGA behavior. These decisions prepare the next task; Task3 implementation remains gated on Task2's independent acceptance and exact final consumed APIs. Main will supply one complete Task3 brief with these choices, not dispatch from a proposal alone.

- Ruling: accept one private FakeConfig-tagged VM map, existing shared task map/UPID sequence, and the additional neutral identity read. Rich configs fail old config downcasts without disappearing from inventory. Preserve old-only behavior and public signatures. Cost if wrong: local compatibility rework, never a second invisible VM map or relaxed old evidence.
- Ruling: use checked private incarnation counters for explicit insert/replace fixture events and capture original source/target incarnations for delayed provisioning work. Compare full before-state as well as digest at admission and completion. New independent power fixture changes also invalidate pending work; QGA flag changes do not. Exact-UPID provisioning completion coexists with old-only complete_pending FIFO. No-op explicit replacement is still a replacement event. Cost if wrong: conservative local task failure; never redirect accepted work to a replacement.
- Ruling: new provisioning duplicate policy rejects every later submit with the same OperationId after the first atomic admission attempt, even if the first was rejected. Record attempts separately from accepted operations; rejection does not consume a queued fault. Keep old-versus-old deduplication behavior unchanged. New pending resource pins block overlap with both families; old submits must respect accepted new pins. This is fake behavior, not durable authorization or proof the controller avoided a resend.
- Ruling: fault selectors use action plus optional operation ID, with derived profile, exact-operation priority and FIFO within each selector. Keep old fault queues and pause controls independent. Accept exact-UPID completion and private immutable submission/acceptance records from the proposal. An inspected fake acceptance never automatically becomes an authoritative controller receipt. Cost if wrong: fixture interface changes, not weakened production recovery policy.
- Ruling: accept additive fixed ContractError variants UnsupportedPveDiskSerial and InvalidPveExpectations, using the proposal's payload-free messages. Derive solely through validated getters and reject desired disk serial over20 ASCII bytes before conversion returns. Existing broad declaration/fact constructors remain unchanged. Cost if wrong: later API refinement; no silent identity truncation or partial conversion.
- Ruling: permit cohesive private helper modules under native_fake/provisioning/ and test helpers under tests/provisioning_support/, retaining one implementation owner. No new dependency or public arbitrary mutation hook. Storage-free values remain independently supplied facts, not an invented thin/thick accounting model; growth checks the delta and minimum-free requirement without turning configuration into resize. QGA remains independent and false by default.

Task3 acceptance must exercise the complete PVE sequence against actual shared-world effects and fresh reads, not only caller-supplied expected snapshots. Include mixed-family collision/inventory visibility, delayed source/target replacement with copied digest/marker, independent boot faults, response-loss accepted counts, exact20/21-byte downward conversion, capacity retention and true growth. Full service process recovery, Windows behavior and installed-PVE compatibility are later gates and cannot be claimed from these tests.
