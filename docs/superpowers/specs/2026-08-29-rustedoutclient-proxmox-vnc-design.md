...
The client advertises both DesktopSize (`-223`) and ExtendedDesktopSize (`-308`). After the session is Ready and a stable viewport is known, Dynamic Resolution may send the standard one-screen SetDesktopSize client message (`251`). The initial client-request subset treats exactly one returned screen at origin `(0,0)` with zero flags as eligible for Applied. A structurally valid bounded server-driven reason-0/reason-2 update with a multi-screen or flagged layout is a nonterminal Unsupported capability result, but its aggregate rectangle-header dimensions still become the real framebuffer geometry so later rectangles are neither cropped nor parsed against stale bounds. Malformed, contradictory, or over-limit layouts fail closed; a rejected or unsupported client-request response does not mutate geometry. Requests are constrained by the same 8,192-by-8,192 and 33,554,432-pixel framebuffer limits, never go below 640-by-480, and are rounded down to whole multiples of eight. The UI uses the viewport's backing-pixel dimensions, with those limits applied, so a Retina window is not silently fixed to its 1,280-by-800 logical-point size.

Viewport changes are debounced for 250 ms. Only one transmitted request lifecycle may be unresolved; a newer desired size replaces the single pending follow-up rather than adding another queue entry. Turning Dynamic Resolution off cancels only unsent debounce/follow-up work, not bookkeeping for a request already on the wire. A timeout may stop automatic retries and show Timed out, but an unanswered wire request remains correlated until its eventual response so Retry cannot cause a stale response to resolve a newer request. A QEMU `Request forwarded` result is Pending, not proof that Windows changed resolution. Success requires a later valid DesktopSize/ExtendedDesktopSize update with the requested dimensions. A rejection, unsupported-layout result, or two-second no-change timeout leaves Fit to Window available. The status bar shows Requested, Pending, Applied, Rejected, Unsupported, or Timed out without disconnecting an otherwise healthy console. Malformed resize messages still fail closed as protocol errors.
...
Keyboard modifiers, pointer buttons, and focus ownership are tracked per native session. Switching tabs, moving focus to app controls, losing window focus, reconnecting, closing, or removing a tab targets key and pointer release to the prior owning session before another session can accept input. In 1:1 mode, pointer input is accepted only from the visible intersection of the framebuffer image and console instrument bay; coordinates still map against the full image, and a button pressed inside receives its release even if the pointer leaves the visible area.

Framebuffer state coalesces a bounded dirty region per session. Texture creation or dimension change performs one full upload; unchanged-size updates extract and submit only the dirty rows through a partial texture update. A one-pixel change therefore does not synchronously rebuild or stage the full maximum-size framebuffer, and two native sessions retain independent dirty state.

Native window closure is coordinated rather than immediate: the UI cancels the platform close while it nonblockingly queues shutdown, drains manager events, and retains the runtime until the bounded session/proxy/master cleanup worker completes. A full command queue is retried on later frames. Only worker-channel completion permits the final viewport close.
