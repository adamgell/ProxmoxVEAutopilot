...
```text
74fec96191e1ec660ed8b30ed30b34f0c6edb8f4
```
...
`rust-controller` runtime tree reports 75 changed paths, with 11,894 additions
and 190 deletions. The changed set includes controller, PostgreSQL store,
fixture IPC, migrations, scheduler, and owned-Linux launcher code. Therefore
the retained `f92a6287` image cannot be relabeled or reused as evidence for
`74fec961`; changing only `CONTROLLER_GIT_SHA` would be an invalid source seal.
...
| Current-source image build | No retained image for `74fec961` | Open |
