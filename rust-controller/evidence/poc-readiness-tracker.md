...
| Owned Linux database fixtures | Source review accepted at a36c166f373331ff0a66324fd9301d6a745c5721 after the required absolute-deadline supervisor repair and independent re-review. See linux-owned-fixture-source-review.md for exact local test scope and unresolved historical native interruption. Final Linux artifact/database execution remains pending |
...
The bounded test-only Linux database backend under `2026-09-05-rust-linux-owned-fixtures.md` is now source-reviewed at `a36c166`. The implementation and supervisor repair have independent review and local covering evidence, with exact limits recorded in `linux-owned-fixture-source-review.md`. An Astra agent is preparing the final Linux gate scripts without executing builds or probes; main identified and returned a runtime stdout/stderr ordering issue before execution. Main must inspect the completed scripts and run the actual four-package Linux database suite, including all explicit Linux failure cases, in a fresh owned PostgreSQL namespace. The prior successful SQL topology probe is feasibility evidence only. Next come durable attempts/deadlines, callbacks, service integration and recovery; the overall goal remains active.

Reliability evidence to retain: the first fixture-change macOS suite reported two native-controller failures, then stranded at an existing unbounded checkpoint wait and was terminated as incomplete. The first failing test subsequently passed unchanged in isolation; the complete native harness passed 44/44 unchanged. These reruns do not establish the original cause. The replacement full suite is currently progressing under a finite watchdog. Preserve the original failure/interruption and later results separately; do not call this a fixed defect or erase it from release-candidate assurance. Before PoC readiness, revisit this uncertainty with the full service/recovery evidence and bounded diagnostic coverage for early controller exit before a test checkpoint. Evidence and exact owned-resource cleanup are retained in the Linux fixture SDD ledger.
