...
| Exact-source Linux | Current source `a607861f24cac19cad4565365df2a29b58a33e0b` built as amd64 image `sha256:7178b72fa5b4d1f8cc1fc1ecf5f6896909f2a3e7eead32733e4a6be450eaaef9`; latest retry was correctly refused at `12,318,252 kB` available, below the 12 GiB guard, while the prior attempt hit the 60-second runner-create deadline | Exact-source build and image seal proven; Linux runtime qualification open |
...
Current scope at `8c43967f`: this branch and PR #65 contain an accumulated
...
The current PR head is `8c43967fe710460a05d183e739d8a27a4cf4e65d` (verified 2026-09-11). The exact-source
...
callback observation-gap audits at `c0d38f46`/`91371a11`, followed by configured
route-provenance retention at `e6394fe6`/`aae58311`, followed by route reload
and legacy-provenance proof at `c7b1bde5`/`ed8f3d4d`, and database-backend death
recovery at `5d346bab`/`6da64fd3`, followed by the controller capture gap
audit at `4dae0ae9`/`c1d9965b` and controller capture integration at
...
At `3a3b70d0`, a fresh predecessor-bound StartPe adapter can restore exact
daemon receipt bytes for observation only; repeated restore/checkpoint/send is
...
At `db84526f`, validated full StartPe publication now maps fixture provisioning
facts for source/target configuration, power/coverage, and media with original
timestamps. Missing publication and unmapped cluster inventory remain
fail-closed; this does not claim Setup, PostgreSQL, or full evaluator parity.
...
`LeaseGrant`, stop authority, and consumed envelope in one legitimate test
fixture. The typed stop outcome APIs and immutable outcome schema therefore
...
At `eb15cca5`, the legacy callback task-observation gap is explicit: existing
...
At `4dae0ae9`, the controller send integration gap is explicit: the generic
`submit_and_capture_once` path still calls only the semantic receipt writer,
while the closed fixture response is available only on the concrete adapter.
...
At `9c09361e`, the implementation-ready stop-outcome design specifies an
immutable fixture outcome record and transaction-owned prepare/record APIs for
...
At `33a60c0c`, sealed shared-history provenance now has a canonical
length-delimited SHA-256 fingerprint derived only from its private operation,
generation, owner, and channel fields. Focused tests prove stability,
...
At `7de11646`, the stop envelope and outbox persist both authority domains:
the scheduler lease generation (`i64`) and supervisor/checkpoint generation
(`Uuid`), along with the exact lease token. Selection and replay derive the
