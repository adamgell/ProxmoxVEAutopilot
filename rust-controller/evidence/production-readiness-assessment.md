...
- Controller integration remains a distinct open seam: `OsDeployController` and its consuming dispatch permit are concretely tied to `NativeFakePve`, while the fixture protocol currently accepts caller-supplied synthetic state and returns no typed mutation receipt. The next proof must establish an opt-in fixture capability and one typed Clone round-trip before any A/B worker-death claim.
- A default-disabled `fixture-ipc` feature now exports reusable test daemon/ledger support (`25e4f56`), and typed Clone request/receipt envelopes are verified at `9ecd6c5`; a macOS Unix-socket timeout portability correction is `4a29b46`. These remain message/fixture contracts only; the concrete controller capability seam and typed end-to-end Clone path are still open.
...
- The consuming dispatch permit seam is committed at `e34eacc`, with Arc caller compatibility fixed in `e8bdd27` and `feaf463`. Postgres-store all-target compilation, 22 doctests, and strict Clippy pass. The runtime database suite remains to be rerun; fixture IPC submission and controller ownership/helper adaptation are still open.
