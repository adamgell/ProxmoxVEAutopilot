-- Closed fixture registration selection, linked to immutable delivered authority.
...
ALTER TABLE rust_controller.osdeploy_decisions ADD CONSTRAINT osdeploy_decisions_action_resolution CHECK (CASE
    ...
    ELSE false END);
CREATE TABLE IF NOT EXISTS rust_controller.fixture_pe_registrations (
    operation_id uuid PRIMARY KEY,
    run_id uuid NOT NULL,
    attempt_id uuid NOT NULL,
    selected_event_id uuid NOT NULL UNIQUE,
    start_operation_id uuid NOT NULL REFERENCES rust_controller.fixture_pe_delivery_exposures(operation_id),
    alias_sha256 bytea NOT NULL REFERENCES rust_controller.fixture_pe_credential_aliases(alias_sha256),
    identity_canonical_json text NOT NULL,
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgrelid='rust_controller.fixture_pe_registrations'::regclass AND tgname='fixture_registration_no_mutation' AND NOT tgisinternal) THEN
        CREATE TRIGGER fixture_registration_no_mutation BEFORE UPDATE OR DELETE ON rust_controller.fixture_pe_registrations FOR EACH ROW EXECUTE FUNCTION rust_controller.reject_native_mutation();
        CREATE TRIGGER fixture_registration_no_truncate BEFORE TRUNCATE ON rust_controller.fixture_pe_registrations FOR EACH STATEMENT EXECUTE FUNCTION rust_controller.reject_native_mutation();
    END IF;
END $$;
