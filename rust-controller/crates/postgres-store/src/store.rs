...
impl PgStore {
    ...
    pub async fn migrate(&self) -> Result<(), StoreError> {
        ...
        #[cfg(feature = "fixture-ipc")]
        sqlx::raw_sql(include_str!(
            "../migrations/0007_fixture_osdeploy_origins.sql"
        ))
        .execute(&self.pool)
        .await?;
        #[cfg(feature = "fixture-ipc")]
        ...
        #[cfg(feature = "fixture-ipc")]
        sqlx::raw_sql(include_str!(
            "../migrations/0009_fixture_delivery_policy.sql"
        ))
        .execute(&self.pool)
        .await?;
        #[cfg(feature = "fixture-ipc")]
        sqlx::raw_sql(include_str!(
            "../migrations/0010_fixture_credential_delivery.sql"
        ))
        .execute(&self.pool)
        .await?;
        #[cfg(feature = "fixture-ipc")]
        {
            ...
            .await?;
            sqlx::raw_sql(include_str!(
                "../migrations/0012_fixture_pe_registration.sql"
            ))
            .execute(&mut *tx)
            .await?;
            sqlx::raw_sql(include_str!(
                "../migrations/0013_fixture_completion_package.sql"
            ))
            .execute(&mut *tx)
            .await?;
            sqlx::raw_sql(include_str!("../migrations/0014_fixture_pe_completion.sql"))
                .execute(&mut *tx)
                .await?;
            sqlx::raw_sql(include_str!("../migrations/0015_fixture_stop_outbox.sql"))
                .execute(&mut *tx)
                .await?;
            sqlx::raw_sql(include_str!(
                "../migrations/0016_fixture_stop_release_outcomes.sql"
            ))
            .execute(&mut *tx)
            .await?;
            sqlx::raw_sql(include_str!(
                "../migrations/0017_fixture_start_pe_responses.sql"
            ))
            .execute(&mut *tx)
            .await?;
            tx.commit().await?;
