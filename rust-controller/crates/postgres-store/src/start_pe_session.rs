...
use osdeploy_adapter::{
    AuthenticatedPeWitnessV1, PeRegistrationAnchorV2, StartPeArmingContextV1, StartPeArmingRefusal,
    StartPeAtomicArmingProposalV1,
};
use sqlx::Row;

fn exact_micros(at: chrono::DateTime<chrono::Utc>) -> Result<u64, Error> {
...
impl PgStore {
    ...
    pub async fn probe_osdeploy_start_pe_session_transaction(
        ...
        let anchor =
            PeRegistrationAnchorV2::from_v1(proposal.anchor()).map_err(|_| Error::Validation)?;
        self.probe_osdeploy_start_pe_session_transaction_v2(proposal.context(), &anchor)
            .await
    }

    /// Exact microsecond diagnostic; v1 callers cannot round durable timestamps
    /// into a matching anchor. No authenticated witness or write path exists.
    pub async fn probe_osdeploy_start_pe_session_transaction_v2(
        &self,
        context: &StartPeArmingContextV1,
        anchor: &PeRegistrationAnchorV2,
    ) -> Result<StartPeArmingRefusal, Error> {
        anchor
            .assess(context, anchor.opened_unix_micros())
            .map_err(|_| Error::Validation)?;
        let mut tx = self.pool().begin().await?;
        let result=async {
            sqlx::query("SET TRANSACTION ISOLATION LEVEL REPEATABLE READ READ ONLY").execute(&mut *tx).await?;
            let c=context; let a=anchor;
            ...
            let micros=|key| -> Result<u64,Error> {exact_micros(row.try_get(key)?)};
            if row.try_get::<uuid::Uuid,_>("anchor_event_id")?!=a.dispatch_event() || row.try_get::<uuid::Uuid,_>("dispatch_event_id")?!=a.dispatch_event()
                ...
                || row.try_get::<String,_>("action")?!="pve_dispatch_committed" || micros("opened_at")?!=a.opened_unix_micros() || micros("dispatched_at")?!=a.opened_unix_micros() || micros("evaluated_at")?!=a.opened_unix_micros()
                || micros("deadline_at")?!=a.deadline_unix_micros() || row.try_get::<i32,_>("budget_seconds")?!=a.budget_seconds() as i32 {return Err(Error::Validation);}
            let checked:chrono::DateTime<chrono::Utc>=row.try_get("checked_at")?;
            let checked=exact_micros(checked)?;
            a.assess(c,checked).map_err(|_|Error::Validation)
        }.await;
        tx.rollback().await?;
        result
    }
...
mod tests {
    ...
    fn original_anchor_precision_is_never_silently_truncated() {
        assert_eq!(
            exact_micros(chrono::DateTime::from_timestamp(1, 123_001_000).unwrap()),
            Ok(1_123_001)
        );
        assert_eq!(
            exact_micros(chrono::DateTime::from_timestamp(1, 123_001_001).unwrap()),
