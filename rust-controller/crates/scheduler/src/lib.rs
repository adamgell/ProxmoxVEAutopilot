...
impl Scheduler {
    ...
    pub async fn heartbeat(&self, grant: &LeaseGrant) -> Result<LeaseGrant, SchedulerError> {
        ...
        lock_operation(&mut transaction, grant.operation_id()).await?;
    ...
    pub async fn request_cancel(
        ...
        append_state_event(
            ...
            StateAppend {
                ...
                semantic_key: format!("scheduler:cancelling:{}", revision + 1),
    ...
    pub async fn reap_expired(&self) -> Result<ReapSummary, SchedulerError> {
        ...
        let operation_ids: Vec<Uuid> = sqlx::query_scalar(
            "SELECT operation_id FROM rust_controller.worker_leases \
             WHERE lease_expires_at <= clock_timestamp() ORDER BY operation_id",
        )
...
async fn lock_lease(
    ...
    let row: Option<(Uuid, String, i64, String, String, bool)> = sqlx::query_as(
        "SELECT attempt_id, executor_kind, generation, worker_id, lease_token, \
                lease_expires_at > clock_timestamp() AND deadline_at > clock_timestamp() \
         FROM rust_controller.worker_leases WHERE operation_id = $1 FOR UPDATE",
    )
    .bind(operation_id.as_uuid())
    .fetch_optional(&mut **transaction)
    .await?;
    let (attempt_id, executor_kind, generation, worker_id, lease_token, active) =
...
async fn append_state_event(
    ...
    let next_revision =
        append
            .revision
            .checked_add(1)
            .ok_or(SchedulerError::InvalidSchedulerTransition {
