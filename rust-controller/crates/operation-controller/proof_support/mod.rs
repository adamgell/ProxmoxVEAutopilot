...
impl Drop for Container {
    fn drop(&mut self) {
        let _ = Command::new("docker")
            .args(["--host", &self.endpoint, "rm", "--force", &self.id])
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .status();
    }
...
impl Fixture {
    pub async fn new() -> Self {
        ...
        assert!(
            ...
            "local_container_invalid"
        );
        let container = Container { id, endpoint };
        let out = Command::new("docker").args(["--host", &container.endpoint,"inspect","--format","{{(index (index .NetworkSettings.Ports \"5432/tcp\") 0).HostIp}}:{{(index (index .NetworkSettings.Ports \"5432/tcp\") 0).HostPort}}", &container.id]).output().expect("local_publish_invalid");
