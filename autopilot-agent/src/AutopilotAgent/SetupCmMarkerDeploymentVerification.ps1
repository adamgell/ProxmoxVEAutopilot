...
if ($env:COMPUTERNAME -ine $deviceName) {
    throw "MECM marker deployment verification may only run on $deviceName."
}
...
for ($attempt = 0; $attempt -lt 24; $attempt++) {
    ...
    $appEnforceMentionsMarker = (Test-Path -LiteralPath $appEnforceLog -PathType Leaf) -and
        ((Get-Content -LiteralPath $appEnforceLog -Tail 500 -ErrorAction SilentlyContinue | Out-String) -match 'LABZ1 MECM Marker')
