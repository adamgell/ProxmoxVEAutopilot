...
if ($Principal.Length -gt 256 -or $Principal -notmatch '^[^\\/:*?""<>|]+\\[^\\/:*?""<>|]+$') {
    throw 'principal must be a DOMAIN\account value.'
}

$computerSystem = Get-CimInstance -ClassName Win32_ComputerSystem -ErrorAction Stop
if (-not $computerSystem.PartOfDomain -or [string]::IsNullOrWhiteSpace($computerSystem.Domain)) {
    throw 'CM01 must be joined to a domain before assigning a console principal.'
}
...
if ($principalSid.AccountDomainSid.Value -ne $domainAdminsSid.AccountDomainSid.Value) {
    throw 'principal must resolve to an account in the CM01 domain.'
}
$principal = $principalSid.Translate([System.Security.Principal.NTAccount]).Value
