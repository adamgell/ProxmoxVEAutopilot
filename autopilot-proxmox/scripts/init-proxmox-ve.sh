...
Options:
  --phase foundation|bootstrap|operational|reset-dev-lab|all
  --resume
  --wait-for-media
  --download-windows
...
download_windows_iso_if_requested() {
  ...
  local iso_storage dir filename source_url resolved_json resolver_error eval_error download_source
...
scan_media() {
  ...
  dir="$(iso_dir_for_storage "${iso_storage}")"
  mkdir -p "${dir}"
  windows_iso="$(find "${dir}" -maxdepth 1 -type f -iname '*.iso' \
    ! -iname 'virtio-win*.iso' ! -iname '*winpe*' ! -iname '*cloudosd*' \
    | awk '{name=tolower($0)} name ~ /win(dows)?|windows_11|win11|cliententerprise|enterpriseeval|client.*eval/ {print; exit}')"
