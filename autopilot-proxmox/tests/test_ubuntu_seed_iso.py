...
# -----------------------------------------------------------------------------
# /api/ubuntu/per-vm-seed endpoint tests (Task 20)
# -----------------------------------------------------------------------------
...
def test_per_vm_seed_builds_and_uploads(tmp_path, monkeypatch):
    ...
    with patch("web.ubuntu_seed_iso.build_seed_iso", side_effect=fake_build), \
        ...
        mock_upload.return_value = MagicMock(status_code=200, ok=True,
                                             text='{"data":null}')
        resp = client.post(
            f"/api/ubuntu/per-vm-seed?vmid=107&sequence_id={ubuntu_plain['id']}"
            f"&hostname=autopilot-PF123"
        )

    assert resp.status_code == 200, resp.text
    data = resp.json()
    assert data["ok"] is True
    # Per-VM ISO name pattern: ubuntu-per-vm-<vmid>.iso
...
def test_per_vm_seed_400_on_windows_sequence(tmp_path, monkeypatch):
    ...
    windows = next(s for s in seqs if s["target_os"] == "windows")
    resp = client.post(
        f"/api/ubuntu/per-vm-seed?vmid=107&sequence_id={windows['id']}&hostname=h"
    )
    assert resp.status_code == 400
    assert resp.json()["ok"] is False
