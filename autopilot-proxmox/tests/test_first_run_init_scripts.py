...
def test_pve_init_provisions_controller_without_host_docker_runtime():
    ...
    assert '"proxmox_api_base": "https://{{ proxmox_host }}:{{ proxmox_port }}/api2/json"' in text
    assert '"proxmox_api_auth_header": "PVEAPIToken={{ vault_proxmox_api_token_id }}={{ vault_proxmox_api_token_secret }}"' in text
...
def test_pve_runtime_config_sync_copies_vault_without_logging_secret():
    ...
    assert "cat \"${VAULT_FILE}\"" not in sync
