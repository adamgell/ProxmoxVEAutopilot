...
def test_job_manager_start_enqueues_pending_row(pg_conn):
    """Task 13: JobManager is enqueue-only. start() writes a pending
    row to Postgres and returns a legacy-shaped dict — no subprocess,
    no thread."""
...
def test_job_manager_start_pretouches_log_file(pg_conn):
...
def test_job_manager_list_and_get_delegate_to_jobs_db(pg_conn):
    from web import jobs
    with tempfile.TemporaryDirectory() as d:
        jobs_dir = Path(d)
        mgr = jobs.JobManager(jobs_dir=str(jobs_dir))
...
def test_job_manager_get_nonexistent_job_returns_none(pg_conn):
    from web import jobs
    with tempfile.TemporaryDirectory() as d:
        jobs_dir = Path(d)
        mgr = jobs.JobManager(jobs_dir=str(jobs_dir))
