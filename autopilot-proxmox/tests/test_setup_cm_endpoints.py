...
def test_console_domain_admins_queues_only_for_cm01(
    pg_conn,
    monkeypatch,
    tmp_path,
):
    ...
    response = client.post(
        "/api/setup-cm/v1/agents/agent-labz1-cm01/console-domain-admins",
        json={},
    )

    assert response.status_code == 202
    assert response.json()["kind"] == "setup_cm_console_domain_admins"
    assert response.json()["request"] == {}


def test_console_domain_admins_rejects_any_other_agent(monkeypatch, tmp_path):
    client = _client_with_artifact_root(monkeypatch, tmp_path)

    response = client.post(
        "/api/setup-cm/v1/agents/agent-other/console-domain-admins",
        json={},
    )
