...
test("provision launch composer keeps hostname previews inside Windows limits", async ({ page }) => {
  ...
  const launchLayout = await launchGrid.evaluate((element) => {
    const gridBox = element.getBoundingClientRect();
    const childBoxes = Array.from(element.children).map((child) => child.getBoundingClientRect());
    const columns = getComputedStyle(element).gridTemplateColumns.split(" ").filter(Boolean).length;
    return {
      columns,
      spillsHorizontally: childBoxes.some((box) => box.left < gridBox.left - 1 || box.right > gridBox.right + 1),
